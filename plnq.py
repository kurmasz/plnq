...
def determine_absolute_output_dir(dir_in, description_filename):
  if not description_filename.endswith('.ipynb'):
      raise ValueError("Parameter description_filename to determine_absolute_output must end with .ipynb")
...
def main():
  ...
  output_dir_name = determine_absolute_output_dir(output_dir_name, description_file_name)
  ...
  default_config = {"ignore": [], "pass_through": []}
  ...
  for current_cell_num in range(1, len(description_json['cells'])):
      ...
      possible_names = re.findall(r"^([^(]+)\(", method_signature)
